...
export class TickerPanel extends Application {
    ...
    async getData(options) {
        ...
        for ( let i = 0; i < tickers.length; i++ )
        {
            ...
            // If the user owns the ticker.
            else if (tickers[i].owner == game.user.id)
    ...
    activateListeners($html) {
        ...
        $html.find("[data-action=delete-ticker]").on("click", async (event) => {
            ...
            if (deleting) {
                this.db.delete(tickerId);
            }
        });

        // Drag/drop reordering
        new SortableJS($html.find(".ticker-list").get(0), {
            animation: 200,
            direction: "vertical",
            ...
            onEnd: (event) => {
                ...
                const numItems = $html.find(".ticker-entry").length;
