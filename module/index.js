...
Hooks.once("socketlib.ready", () => {
    ...
    window.tickerSocket.register("deleteTicker", (id)=> {
        tickerDatabase.delete(id, false);
    })
    window.tickerSocket.register("moveTicker", (id, idx) => {
        tickerDatabase.move(id, idx, false);
    })
})
...
Hooks.on("createSetting", (setting) => {
    if (setting.key === "simple-tickers.gmTickers") {
        window.tickerDatabase.refresh();
    }
    if (setting.key === "simple-tickers.userTickers") {
        window.tickerDatabase.refresh();
    }
});
